...
const MyNavScreen = ({navigation, banner}) => (<ScrollView>
    <SafeAreaView forceInset={{
            horizontal: 'always'
        }}>
        <SampleText>{banner}</SampleText>
        <Button onPress={() => navigation.navigate('Profile', {name: '吴凯'})} title="Open 吴凯profile screen"/>
        <Button onPress={() => navigation.navigate('NotifSettings')} title="Open notifications screen"/>
        <Button onPress={() => navigation.navigate('MainTab')} title="Go to MainTab tab"/>
...
const MySettingsScreen = ({navigation}) => (<View>
    <Button onPress={() => navigation.navigate('MainTab')} title="Go to MainTab tab"/>
</View>);
